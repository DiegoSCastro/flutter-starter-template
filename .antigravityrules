...
Official task-playbook skills are vendored under `.agents/skills/` (hash-pinned via `skills-lock.json`).
When a task matches a skill name, invoke the skill rather than improvising—it encodes the team's preferred workflow:
*   Setting up routing → `flutter-setup-declarative-routing`
*   JSON serialization → `flutter-implement-json-serialization`
*   Adding widget/unit tests → `flutter-add-widget-test` / `dart-add-unit-test`
*   Running static analysis → `dart-run-static-analysis`
*   Fixing runtime errors → `dart-fix-runtime-errors`
...
* **Navigation:** Use a modern routing package like `auto_route` or `go_router`. For more guidelines around navigation, see the section on [routing](#routing).
...
* **Text Fields:** Configure `textCapitalization`, `keyboardType`, and `placeholder`.
